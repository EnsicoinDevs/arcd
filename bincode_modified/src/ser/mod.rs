...
impl<'a, W: Write, O: Options> Serializer<W, O> {
    fn serialize_varuint(&'a mut self, v: u64) -> Result<()> {
        if v <= 252 {
            self.serialize_u8(v as u8)
        } else if v <= 0xFFFF {
            let s: u8 = 0xFD;
            try!(self.serialize_u8(s));
            self.serialize_u16(v as u16)
        } else if v <= 0xFFFFFFFF {
            try!(self.serialize_u8(0xFE));
            self.serialize_u32(v as u32)
        } else {
            try!(self.serialize_u8(0xFF));
...
impl<'a, W: Write, O: Options> serde::Serializer for &'a mut Serializer<W, O> {
    ...
    fn serialize_str(self, v: &str) -> Result<()> {
        try!(self.serialize_varuint(v.len() as u64));
    ...
    fn serialize_seq(self, len: Option<usize>) -> Result<Self::SerializeSeq> {
        ...
        try!(self.serialize_varuint(len as u64));
